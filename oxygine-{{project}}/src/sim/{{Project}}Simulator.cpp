...
DECLARE_SMART({{Project}}Dialog, sp{{Project}}Dialog);
class {{Project}}Dialog : public Actor
{
public:
    enum
    {
...
//Json::Value _{{project}}(Json::objectValue);

/*void {{project}}SimulatorLogin()
{
    sp{{Project}}Dialog dialog = new {{Project}}Dialog;
    ...
    dialog->_btnOk->addClickListener([ = ](Event * e)
    {
        ...
        {{project}}::internal::loginResult(true);
    });
...
void {{project}}SimulatorInit()
{
    log::messageln("{{Project}} Simulator Init");


    /*file::buffer bf;
    file::read(".{{project}}", bf, ep_ignore_error);

    if (!bf.empty())
    {
        Json::Reader reader;
        reader.parse((char*)&bf.front(), (char*)&bf.front() + bf.size(), _{{project}}, false);
    }*/
}
